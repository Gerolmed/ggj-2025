...
void fish_death(Pufferfish* fish, GameState* state)
{
    ...
    directions[4] = Vector2Normalize(Vector2(1, 1));
    directions[5] = Vector2Normalize(Vector2(-1, 1));
    directions[6] = Vector2Normalize(Vector2(1, -1));
    directions[7] = Vector2Normalize(Vector2(-1, -1));
...
void fish_update(Pufferfish* fish, GameState* state)
{
    ...
    if (fish_get_radius(fish) < .4) shadow_size = ShadowSize_Smaller;
    else if (fish_get_radius(fish) < .5) shadow_size = ShadowSize_Small;
    else if (fish_get_radius(fish) < .7) shadow_size = ShadowSize_Medium;
    else shadow_size = ShadowSize_Large;

    RenderAnimatedEntity(
        Model_Fish, Vector2(fish->position.x, fish->position.y), 180 + fish->rotation * 180 / PI,
        2 * fish_get_radius(fish), animation, fish->animation_frame % animation->frameCount,
